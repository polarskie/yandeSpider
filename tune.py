...
if __name__ == "__main__":
    ...
    print("loading mode")
    if len(sys.argv) <= 3:
        print("must provide train and test file list and model file")
        quit()
    if len(sys.argv) == 3:
        ...
        tmp = [s for s in input("input layer name to match").strip().split(' ') if s != '']
        for layer in model.layers:
            if any([t in layer.name for t in tmp]):
                layer.trainable = True
        for layer in model.layers:
            print(layer.name, layer.trainable)
        model.compile(keras.optimizers.Adam(lr=0.00001), keras.losses.categorical_crossentropy)
    ...
    for batch_i in range(10000):
        ...
        if batch_i % 100 == 0:
            ...
            if batch_i % 100 == 0:
                ...
                model.save("fine_%s.h5" % time_str)
